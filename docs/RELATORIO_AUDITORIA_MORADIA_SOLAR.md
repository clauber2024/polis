...
- A migration `0014` cria um **Índice de Segurança da Posse** (pesos proprio=1,0 / alugado=0,5 / cedido=0,0 aplicados sobre os percentuais de domicílio de cada tipo, que são armazenados em escala 0–100 — o índice resultante fica, portanto, em escala **0–100**, não 0–1; confirmado pelo comentário da migration `0028`, que documenta explicitamente "indice_seguranca_posse (...) e POSITIVO (0 a 100, maior = melhor)" — correção de 26/07/2026, ver `docs/ANEXO_TECNICO_ATLAS_SOLAR_JUSTO.md`, Bloco 1, item 3) e um **Índice de Precariedade Habitacional** (cortiço + parede inadequada + população em favela, escala 0–1) como objetos de primeira classe do schema — não derivados ad hoc em uma consulta pontual, mas materializados em view (`vw_indices_compostos_moradia_infraestrutura`) para consumo recorrente por qualquer análise futura.
...
**Ressalva (26/07/2026):** este segundo padrão é, até hoje, uma hipótese conceitual, não um padrão confirmado estatisticamente. O único teste quantitativo direto de `percentual_apartamento` sobre a adoção de MMGD residencial encontrado nos materiais (controlando renda, ver ARQUITETURA.md, seção "Índices compostos e metodologia de cruzamentos") produziu coeficiente parcial **positivo** (+0,115 a +0,156) — o oposto do que "mais apartamento, menos telhado próprio, menos adoção" preveria. A leitura mais provável é que `percentual_apartamento` funcione como proxy de porte/modernidade urbana, não como medida limpa de barreira física ao telhado individual. Ver `docs/ANEXO_TECNICO_ATLAS_SOLAR_JUSTO.md`, Seção 10.3, para o detalhe completo.
...
**Achado 2 — quando o programa chega a um Vazio de Acesso, chega com menos intensidade per capita do que chega aos municípios já bem servidos.** R$ 1.410,93/1.000 hab. nos vazios é 42,2% menor que nos municípios de "Adoção acima do potencial" (R$ 2.441,27) e 30,6% menor que em "Acesso pleno" (R$ 2.033,24). O padrão de A se repete em B: não é só menos frequente, é também menos intenso. (Correção de 26/07/2026: a versão original deste achado citava "31% menor" na primeira comparação — erro de cálculo, recálculo direto a partir dos mesmos dois valores da tabela confirma 42,2%, ver `docs/ANEXO_TECNICO_ATLAS_SOLAR_JUSTO.md`, Bloco 1, item 1.)
