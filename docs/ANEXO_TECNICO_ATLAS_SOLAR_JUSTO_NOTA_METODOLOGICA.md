...
**Nota sobre a origem deste documento.** O texto abaixo foi produzido a partir de três materiais do projeto Atlas Solar Justo: a auditoria técnica da base de dados e da metodologia, a síntese executiva voltada a tomadores de decisão e, incorporado numa segunda rodada de revisão, o rascunho do relatório principal — "Atlas das experiências de MMGD solar: Caminhos para democratizar o acesso à energia solar distribuída no Brasil" (Instituto Pólis, versão ainda com lacunas de redação marcadas "xx" e data de capa em aberto) —, complementados por trechos dos registros técnicos de desenvolvimento da plataforma e do registro de decisões metodológicas do projeto quando necessário para esclarecer um procedimento citado nos demais. **Não foram localizadas, em nenhum dos materiais, Notas Metodológicas 1/2/3 individualizadas, arquivos de mapas/gráficos/tabelas prontos (além dos que o próprio rascunho do relatório principal referencia como "Figura X", ainda sem o objeto correspondente produzido), nem uma lista de referências bibliográficas formal.** Onde a tarefa original pedia a verificação de um dado que simplesmente não consta em nenhum dos materiais disponíveis, isso é sinalizado com as marcações combinadas ([informação a confirmar] etc.), nunca preenchido por inferência.
...
1. **Diferença percentual de intensidade financeira incorreta.** Uma versão preliminar deste texto afirmava que o valor per capita em Vazios de Acesso (R$ 1.410,93/1.000 hab.) é "31% menor" que em municípios de "Adoção acima do potencial" (R$ 2.441,27/1.000 hab.). O recálculo direto a partir dos mesmos dois valores — (2.441,27 − 1.410,93) / 2.441,27 — resulta em **42,2%**, não 31%. A segunda comparação do mesmo parágrafo ("30% menor que em Acesso pleno", R$ 2.033,24) está correta: (2.033,24 − 1.410,93) / 2.033,24 = 30,6%. **[revisão estatística necessária]** — mantido no anexo com o valor recalculado e a divergência explicitada, não substituído silenciosamente.
2. ~~**Denominador divergente para "municípios sem contrato" dentro do mesmo relatório.**~~ **[RESOLVIDO em 26/07/2026, por verificação direta na base de dados do Atlas]** A tabela de distribuição por quadrante (Seção 3.3-A) usa n = 4.476 para municípios sem contrato do Reforma Casa Brasil Solar. A tabela de precariedade habitacional (Seção 3.3-C, "Base completa") usa n = 4.480 para o mesmo grupo, no mesmo documento — diferença de exatamente 4 municípios. **Causa raiz confirmada**: a tabela A opera sobre o universo "classificável" de Vazios de Acesso (5.569 municípios, que exige dado de irradiação solar), enquanto a tabela C (precariedade habitacional) opera sobre o universo completo de municípios da base de dados do Atlas (5.573), que não depende de irradiação. A diferença de 4 é exatamente os 4 municípios sem dado de irradiação no Atlas INPE 2017, confirmados por consulta direta à base de irradiação solar do Atlas: Fernando de Noronha (PE), Boa Esperança do Norte (MT, desmembrado de Sorriso/Nova Ubiratã em 2025) e dois polígonos de corpo d'água tratados como unidade territorial na malha oficial do IBGE — "Área Operacional Lagoa Mirim" e "Área Operacional Lagoa dos Patos" (RS). Mesma causa raiz do item 4 abaixo (universo do IVSH).
3. ~~**Escala do Índice de Segurança da Posse não reconciliada entre as duas descrições que o próprio material fornece.**~~ **[RESOLVIDO em 26/07/2026]** Uma versão preliminar deste texto definia o índice, na descrição original da metodologia, com pesos próprio = 1,0 / alugado = 0,5 / cedido = 0,0, sem explicitar a escala resultante. Já a fórmula do IVSH, no mesmo texto, usa o termo "1 − Índice de Segurança da Posse/100" — o que só faz sentido se o valor estiver em escala 0–100 — e a tabela de quintis por faixa de vulnerabilidade (Seção 6.4) exibe valores entre 53,99 e 97,31, consistentes com uma escala 0–100. **Confirmado por verificação técnica direta na metodologia implementada**: a documentação da metodologia de cálculo do IVSH declara explicitamente que o Índice de Segurança da Posse "é positivo (0 a 100, maior = melhor)" — os pesos são aplicados sobre percentuais de domicílio armazenados em escala 0–100, não sobre uma escala fracionária. A escala real é **0–100**. Este texto foi corrigido na mesma data para explicitar isso.
4. ~~**Universo "5.570" vs. "5.569" vs. "5.573" usado em três lugares diferentes sem nota unificadora.**~~ **[RESOLVIDO em 26/07/2026, por verificação direta na base de dados do Atlas]** Uma versão preliminar deste texto citava "~5.570 municípios" como cobertura nacional geral (aproximação desatualizada); a Matriz de Vazios de Acesso e a auditoria do Reforma Casa Brasil Solar operam sobre uma base de **5.569** municípios "classificáveis"; o IVSH foi calculado para **5.573** municípios. **Causa raiz confirmada por verificação direta na base de dados do Atlas e na malha territorial oficial do IBGE**: a base de municípios do Atlas tem hoje **5.573 registros** — não 5.570 —, número que bate exatamente com a contagem de municípios da malha municipal oficial do IBGE (edição 2025), sem nenhum código duplicado. "~5.570" é uma aproximação desatualizada em relação a essa malha municipal mais recente, que já reflete municípios criados/desmembrados posteriormente (ex.: Boa Esperança do Norte/MT). O universo de **5.569 classificáveis** é 5.573 menos os 4 municípios sem dado de irradiação solar (ver item 2, acima) — não, como a auditoria original supôs, por município(s) pendente(s) de reprocessamento de dado de geração solar residencial (verificado diretamente no sistema do Atlas em 26/07/2026: esse indicador está zerado hoje; a exclusão real decorre da ausência de irradiação, não de geração solar residencial — a explicação anterior estava desatualizada). O IVSH (5.573) simplesmente usa o universo completo da base de municípios, sem excluir os 4 municípios sem irradiação (o IVSH não depende desse dado). Não há município duplicado, extinto ou espúrio na base.

### 1.2 Divergência entre narrativa e evidência estatística (a mais relevante para o rigor do anexo)

5. **O "Padrão 2 — Alta verticalização" do Descompasso Morfológico é apresentado nos dois documentos-fonte como um padrão estabelecido de barreira à adoção solar, mas o único teste quantitativo direto da variável correlata (percentual de domicílios do tipo apartamento, Tabela SIDRA 9928) encontrado nos materiais aponta na direção oposta.** Os registros técnicos de desenvolvimento da plataforma (achado de 06/07/2026) reportam que, controlando por renda, o coeficiente parcial dessa variável sobre a adoção de geração solar residencial foi **positivo** (+0,115 a +0,156, a depender da variante), contrariando a hipótese "mais apartamento = menos telhado próprio = menos adoção". A interpretação registrada no próprio material é que essa variável funciona mais como proxy de porte/modernidade urbana do que como medida limpa de barreira física ao telhado individual. Nenhuma versão preliminar deste texto mencionava esse resultado ao apresentar a "alta verticalização" como segundo padrão de descompasso morfológico. **[inconsistência a esclarecer] / [revisão estatística necessária]** — o anexo trata a verticalização como hipótese conceitual ainda não confirmada estatisticamente, não como padrão validado, e relata o sinal contrário encontrado.
6. ~~**Não foi localizado, em nenhum dos materiais, um critério de corte formal (ex.: "mais de 50% dos domicílios do tipo apartamento") usado para classificar municípios em "alta verticalização"**~~ **[RESOLVIDO em 26/07/2026, por verificação direta na base de dados do Atlas]** Essa variável aparece nos materiais apenas como covariável de controle em testes exploratórios de outliers regionais (Sul e Centro-Oeste), nunca como eixo de uma classificação binária de "descompasso por verticalização" com um limiar definido. O corte de 50% citado na tarefa original não foi encontrado nos materiais originais — mas foi testado diretamente contra a base de dados do Atlas para avaliar sua utilidade prática: a distribuição nacional do percentual de domicílios em apartamento é extremamente concentrada em valores baixos (mediana de 0,75%; 90º percentil de apenas 8,0%; máximo nacional de 67,3%) — **apenas 3 dos 5.570 municípios do país têm 50% ou mais dos domicílios do tipo apartamento** (Santos/SP, Balneário Camboriú/SC, São Caetano do Sul/SP), e **nenhum dos três tem irradiação solar acima da mediana nacional** — ou seja, um corte de 50% cruzado com irradiação alta identificaria **zero municípios** para o "Padrão 2" de Descompasso Morfológico, tornando esse critério estatisticamente inútil em escala nacional. Um corte mais permissivo de 30% já identifica 38 municípios com alta verticalização, dos quais 8 também têm irradiação acima da mediana nacional — um universo pequeno, mas não vazio. **Conclusão**: um critério formal de corte é tecnicamente possível de definir, mas qualquer valor próximo ao intuitivo "maioria dos domicílios" (50%) é numericamente quase vazio no Brasil — reforçando, por um caminho diferente do teste de correlação (item 5), que a "alta verticalização" não é hoje um padrão nacional generalizável.
...
8. ~~Valores individuais de índice de precariedade habitacional para Jaboatão dos Guararapes, Cabo de Santo Agostinho e Olinda não são informados separadamente~~ **[RESOLVIDO em 26/07/2026, por verificação direta na base de dados do Atlas]** — Jaboatão dos Guararapes: 0,2241; Cabo de Santo Agostinho: 0,2218; Olinda: 0,1499.
9. ~~Valores individuais de índice de precariedade e de IVSH para Aldeias Altas, Buriti e Mirador (MA) não são informados separadamente~~ **[RESOLVIDO em 26/07/2026]** — precariedade habitacional: Aldeias Altas 0,1510, Buriti 0,1347, Mirador 0,1346. IVSH: Aldeias Altas 0,2861, Buriti 0,2910, Mirador 0,2908 — os três teriam, de fato, IVSH próximo ao dos cinco municípios já nomeados como "top 5" nacional (Seção 11.10), mas nenhum entra no top 5 exato.
10. ~~IVSH individual, posição no ranking e eventual ocorrência de empates não são informados para nenhum município além do "top 5"~~ **[RESOLVIDO PARCIALMENTE em 26/07/2026]** — posição no ranking nacional de IVSH (1º = mais vulnerável, entre 5.573 municípios com IVSH calculado): Buriti = 34º; Mirador = 35º; Aldeias Altas = 49º; Jaboatão dos Guararapes = 330º; Cabo de Santo Agostinho = 556º; Olinda = 1.795º. O ranking completo (além desses seis casos) continua **[resultado não informado]** neste anexo — pode ser gerado sob demanda a partir da mesma consulta.
11. ~~Para a variável Segurança da Posse: os materiais não especificam se o indicador computa apenas domicílios próprios/alugados/cedidos ou se também incorpora ocupações/irregularidade fundiária~~ **[RESOLVIDO em 26/07/2026, por verificação direta na metodologia implementada]** — a fonte é a Tabela SIDRA 9928 do Censo 2022 (classificação "Condição de ocupação do domicílio"), que só distingue três categorias: próprio, alugado e cedido. **Não existe, nessa classificação do Censo, uma categoria separada de ocupação irregular/invasão** — "cedido" é tratado, por interpretação editorial (não por definição censitária), como a categoria mais próxima de informalidade. O denominador é confirmado como o total de domicílios da mesma classificação (categoria "Total" da Tabela 9928), ou seja, o total de domicílios particulares recenseados no município nessa tabela — não uma estimativa populacional indireta.
12. **[RESOLVIDO PARCIALMENTE em 26/07/2026, por verificação técnica direta na metodologia implementada]** Sobre os valores financeiros do Reforma Casa Brasil Solar (R$ 61,4 milhões): confirmado que são **valores nominais** — o procedimento de carga soma diretamente o valor liberado de cada linha do extrato original, sem nenhuma etapa de correção monetária ou deflacionamento. Sobre contratos cancelados/duplicados: confirmado que **o extrato original da Caixa Econômica Federal não traz nenhum campo de status/cancelamento** — as colunas disponíveis são período, município, UF, faixa do programa, quantidade de contratos e valores; se contratos cancelados já foram excluídos a montante pela Caixa antes da geração do extrato, isso não pode ser verificado com os materiais disponíveis. **[a confirmar diretamente com a Caixa Econômica Federal]**. Sobre a data exata de extração: permanece **[informação a confirmar]** — só o fim do período coberto (30/04/2026) está registrado, não o dia em que o extrato foi de fato gerado pelo sistema da Caixa. Município sem identificação de código IBGE: não se aplica — a fonte não traz código IBGE em nenhum registro (usa nome + UF), e o casamento por nome normalizado teve 1.091 de 1.093 casos resolvidos automaticamente, 2 por exceção de grafia documentada (grafias antigas de dois municípios, hoje com nome oficial atualizado).
13. **[RESOLVIDO PARCIALMENTE em 26/07/2026, por verificação direta na base de dados do Atlas]** Sensibilidade da Matriz de Vazios de Acesso a um corte mais estrito que a mediana: testando um corte por **terços** (irradiação no terço superior **e** adoção no terço inferior, em vez de acima/abaixo da mediana), o conjunto de "Vazio de Acesso" cai de 1.451 para **706 municípios** — mas os 706 estão **inteiramente contidos** nos 1.451 originais (nenhum município aparece só no corte por terços), ou seja, a mediana amplia o critério original sem descaracterizar o núcleo mais extremo do fenômeno. Sensibilidade dos pesos do IVSH: testando duas ponderações alternativas às razoáveis (50% peso social / 25% precariedade / 25% posse; e 60% precariedade / 20% social / 20% posse) contra os pesos iguais (1/3 cada) hoje usados, a correlação de postos entre o ranking original e os dois rankings alternativos é muito alta em toda a base (0,95 e 0,95, respectivamente) — a ordenação geral é robusta a variações razoáveis de peso. No entanto, olhando só para os 20 municípios de maior IVSH (o recorte mais citado em comunicação pública), apenas 9 dos 20 permanecem no top 20 sob qualquer uma das duas ponderações alternativas — o "topo" da lista é mais sensível a escolhas de peso do que o ranking como um todo. Os cinco municípios já nomeados publicamente como "top 5" (Alto Alegre, Amajari, Uiramutã, Marajá do Sena, Cumaru do Norte) permanecem, todos, dentro do top 20 sob as duas ponderações alternativas testadas — a lista de nomes citados é razoavelmente robusta, ainda que a ordem entre eles mude. Autocorrelação espacial entre municípios vizinhos e colinearidade formal entre renda e irradiação continuam **[teste de sensibilidade não realizado]** — não executados nesta verificação.
14. ~~A região que diverge no sinal da correlação parcial controlada por renda e irradiação simultaneamente não é identificada nominalmente~~ **[RESOLVIDO em 26/07/2026, por reexecução direta do procedimento de correlação com quebra regional]** O teste original grava apenas "quantas regiões mantêm o sinal", não qual diverge — refeito com saída por região, o resultado é: para a **precariedade habitacional**, a região divergente é o **Nordeste** (ρ parcial = +0,0356, mas **não significativo**, p = 0,131 — na prática, associação nula nessa região, não um sinal forte invertido); para a **segurança da posse**, a região divergente é o **Sul** (ρ parcial = +0,1884, **estatisticamente significativo**, p ≈ 5,6×10⁻¹¹) — essa é a mesma divergência regional que um teste bivariado anterior (controle só por renda) já havia identificado para a região Sul, agora **confirmada também** no teste com controle conjunto de renda e irradiação. Ou seja: no Sul do país, ao contrário do padrão nacional, mais segurança da posse está associada a **mais** adoção solar residencial, não menos — um resultado regional genuinamente distinto do resto do país, não um ruído estatístico.

### 1.4 Linguagem que exige suavização antes de entrar no anexo

15. O termo **"ortogonalidade"** era usado, numa versão preliminar deste texto, para descrever a relação entre precariedade habitacional e a classificação de Vazio de Acesso, sem nenhum teste estatístico de ortogonalidade/independência entre as duas variáveis — apenas uma comparação descritiva de percentuais. O termo foi removido e substituído por linguagem descritiva ("dimensões que não coincidem sistematicamente", sem reivindicar independência estatística formal).
16. Expressões como "prova", "confirma definitivamente" e "isso demonstra... que são dimensões parcialmente independentes", presentes numa formulação preliminar, foram suavizadas para refletir o alcance real de uma comparação de percentuais — não um teste de hipótese formal de independência.
17. A afirmação "O Instituto Pólis desenvolveu e validou um novo instrumento" foi mantida, mas qualificada: os próprios materiais preliminares esclarecem que IVSH e Vazio de Acesso são "construções metodológicas originais... ainda não submetidas a validação externa por pares" — este texto usa "validação interna" para não sugerir revisão por pares que não ocorreu.
...
### 1.5 Achados da incorporação do relatório principal (rascunho ainda em redação)
...
24. **A mesma imprecisão de "cerca de 30%" identificada no Bloco 1, item 1, está presente no próprio texto do relatório principal, não apenas nos materiais internos de auditoria.** O trecho atual é: *"R$ 1.410,93 por 1.000 habitantes, cerca de 30% menos que nos municípios de acesso pleno (R$ 2.033,24) ou de adoção acima do potencial (R$ 2.441,27)"* — aplica um único percentual aproximado a duas comparações que, recalculadas, são 30,6% e 42,2% respectivamente. **Texto pronto para substituir o trecho no rascunho do relatório principal:** *"R$ 1.410,93 por 1.000 habitantes, 30,6% menos que nos municípios de acesso pleno (R$ 2.033,24) e 42,2% menos que nos de adoção acima do potencial (R$ 2.441,27)."* **[revisão pendente — aplicar apenas no rascunho externo, já corrigido nas versões preliminares deste texto]**.
25. **O termo "ortogonal"**, sinalizado no Bloco 1, item 15, para os materiais internos, **também aparece no relatório principal** — trecho atual: *"essa correlação é ortogonal ao eixo que define Vazio de Acesso"*. **Texto pronto para substituir:** *"essa correlação não coincide sistematicamente com o eixo que define Vazio de Acesso"*. A recomendação de suavização (Seção 14.7 do anexo) aplica-se diretamente ao texto que será publicado, não apenas aos bastidores.
26. **O achado de sinal invertido da variável de segurança da posse da terra (ρ parcial = −0,2976, oposto ao hipotetizado) não aparece em nenhum lugar do relatório principal.** O texto cita apenas o coeficiente da precariedade habitacional ("p ≈ 2,7×10⁻³⁰... sinal consistente em quatro das cinco regiões do país") ao afirmar robustez estatística da Matriz e do IVSH — sem mencionar que um segundo teste, sobre uma variável usada no próprio IVSH, produziu resultado na direção oposta à esperada. Isso reforça a recomendação do Bloco 6, achado 5: omitir esse resultado do texto publicado é uma escolha editorial que reduz a transparência que o próprio relatório principal reivindica em sua seção de recomendações ("Dados, transparência e desenvolvimento metodológico"). **Parágrafo pronto para inserir logo após o trecho sobre precariedade habitacional:** *"Um segundo teste, sobre a segurança da posse da terra — variável também usada no IVSH —, produziu resultado na direção oposta à esperada: controlando renda e irradiação, municípios com maior segurança da posse (mais domicílios próprios) têm, em média, menor adoção solar residencial (correlação parcial de −0,30; estatisticamente muito significativa). Esse achado é preservado sem suavização; uma hipótese ainda não testada é que reflita composição urbana, com aluguel concentrado em áreas mais centrais e verticalizadas, onde a instalação solar individual já é mais difícil por outros motivos. Chama atenção, adicionalmente, que esse padrão se inverte na região Sul do país, onde mais segurança da posse está associada a mais adoção solar — o oposto do padrão nacional, ainda sem explicação identificada."*
27. **O relatório principal confirma, e não contradiz, a leitura de que a hipótese de "alta verticalização" carece de sustentação estatística direta** — o texto a descreve apenas qualitativamente ("municípios com muitos prédios e condomínios, onde a barreira não é a precariedade, mas a necessidade de uma decisão coletiva"), sem números, sem corte, sem contagem de municípios e sem menção ao sinal positivo encontrado no teste exploratório da variável de percentual de apartamento (Bloco 1, item 5). A ausência de números nesse trecho é, na prática, consistente com a lacuna identificada — o relatório principal não afirma algo que os dados contradizem, apenas não testa a hipótese que enuncia.
...
- Classificar os municípios brasileiros segundo a relação entre potencial de irradiação solar e adoção residencial de energia solar distribuída — MMGD, sigla de Micro e Minigeração Distribuída, a modalidade regulatória sob a qual a ANEEL registra esses sistemas (Matriz de Vazios de Acesso).
...
### 5. Desenho metodológico integrado da análise
...
- **Condição física da moradia** — índice de precariedade habitacional (Censo IBGE 2022 + Cadastro Nacional de Favelas e Comunidades Urbanas), eixo do Descompasso Morfológico (Padrão 1) e um dos três componentes do IVSH.
...
A unidade de análise em todos os três instrumentos é o **município** (5.573 municípios brasileiros no total nacional, conforme a malha municipal oficial do IBGE em sua edição mais recente — números anteriores a essa atualização territorial, como "~5.570", ficam desatualizados). Não há, nos materiais, leitura confiável em nível de bairro, setor censitário ou domicílio individual em escala nacional — um piloto de setor censitário existe apenas para o município de São Paulo, e é, pelos próprios materiais, **sintético**: distribui um total municipal real proporcionalmente pela área de uma grade artificial, não uma medição real desagregada, e não deve ser usado para nenhuma conclusão sobre desigualdade intramunicipal.

O universo efetivamente classificado pela Matriz de Vazios de Acesso é de **5.569 municípios** — o total de **5.573** menos os 4 municípios sem dado de irradiação solar no Atlas INPE 2017: Fernando de Noronha (PE), Boa Esperança do Norte (MT, desmembrado de Sorriso/Nova Ubiratã em 2025), e dois polígonos de corpo d'água tratados como unidade territorial na malha oficial do IBGE — "Área Operacional Lagoa Mirim" e "Área Operacional Lagoa dos Patos" (RS). O IVSH foi calculado para os **5.573 municípios** — o universo completo, sem essa exclusão, porque o IVSH não depende de dado de irradiação.
...
Os períodos de referência divergem por fonte: irradiação solar é uma climatologia de 1999–2015 (INPE/LABREN); os dados de moradia e tipologia habitacional são do Censo IBGE 2022; a MMGD residencial (ANEEL) reflete um levantamento acumulado mais recente disponível, sem data de referência única explicitada neste anexo; os dados do Reforma Casa Brasil Solar cobrem novembro de 2025 a abril de 2026. Essa diferença temporal entre fontes é uma limitação estrutural, tratada na Seção 18.
...
| ANEEL | Microdados de MMGD (classe residencial) | Levantamento mais recente disponível | Potência residencial instalada, número de unidades consumidoras residenciais | Municipal (normalizado por 1.000 hab.) | Eixo de adoção (Matriz de Vazios de Acesso, variável dependente dos testes estatísticos) | Levantamento acumulado, não série temporal; população municipal é estimada, não contagem direta de censo |
...
| IBGE — Cadastro Nacional de Favelas e Comunidades Urbanas | Favelas e Comunidades Urbanas | Não especificado nos materiais | Número e percentual de população em favela/comunidade urbana | Municipal | Componente do índice de precariedade habitacional | Cobertura e metodologia de identificação não detalhadas nos materiais deste anexo |
...
Mediana nacional de irradiação: **5,02 kWh/m²/dia** (o dado de maior precisão localizado nos registros técnicos de desenvolvimento da plataforma registra 5,015 kWh/m²/dia — diferença de arredondamento, não de fonte). Mediana nacional de adoção residencial: **111,3 kW por 1.000 habitantes** (111,29 kW/1.000 hab. na fonte de maior precisão). Sobre a base de 5.569 municípios classificáveis:
...
A Matriz é uma classificação **relativa**, construída a partir da distribuição nacional observada — não um limiar físico ou universal de viabilidade solar. Um "Vazio de Acesso" não é, por si, prova de barreira habitacional: a classificação não controla renda, tarifa, fila de conexão ou qualquer outro fator. A população usada como denominador é estimada, não censitária. A classificação exclui os 4 municípios sem dado de irradiação (Seção 6). Um teste de sensibilidade a um corte mais estrito (terços, em vez de mediana) foi realizado em 26/07/2026: o conjunto de Vazio de Acesso cai de 1.451 para 706 municípios, mas os 706 estão inteiramente contidos nos 1.451 originais — a mediana amplia o critério sem descaracterizar o núcleo mais extremo do fenômeno (ver Seção 17).
...
Indicadores usados: percentual de domicílios em cortiço, percentual de domicílios com parede externa inadequada (Censo IBGE 2022) e percentual/número de população em favelas e comunidades urbanas (Cadastro Nacional de Favelas e Comunidades Urbanas). Os três componentes formam o índice de precariedade habitacional (normalização min–max nacional, escala 0–1), usado tanto no Descompasso Morfológico quanto como componente do IVSH.

#### 10.3 Alta verticalização

A variável disponível para esta hipótese é o percentual de domicílios do tipo apartamento (Censo IBGE 2022, Tabela SIDRA 9928). **Diferente do que a formulação inicial deste trabalho presumia, os materiais originais não continham um critério de corte formal (por exemplo, "mais de 50% dos domicílios"), nem uma contagem de municípios classificados sob esse critério.** Essa lacuna foi testada diretamente em 26/07/2026: a distribuição nacional do percentual de domicílios em apartamento é extremamente concentrada em valores baixos — mediana de 0,75%, 90º percentil de apenas 8,0%, máximo nacional de 67,3%. **Apenas 3 dos 5.570 municípios do país têm 50% ou mais dos domicílios do tipo apartamento** (Santos/SP, Balneário Camboriú/SC, São Caetano do Sul/SP), e **nenhum dos três tem irradiação acima da mediana nacional** — um corte de 50% cruzado com irradiação alta identificaria, portanto, **zero municípios** para este padrão. Um corte mais permissivo de 30% identifica 38 municípios com alta verticalização, dos quais 8 também têm irradiação acima da mediana — um universo pequeno, mas não vazio. Antes dessa variável aparecer nos materiais apenas como covariável de controle em investigações exploratórias de outliers regionais, nenhuma dessas contagens havia sido calculada.

Mais relevante: o único teste quantitativo direto encontrado nos materiais (percentual de domicílios do tipo apartamento sobre adoção de MMGD residencial, controlando renda) produziu coeficiente parcial **positivo** (+0,115 a +0,156), o oposto do que a hipótese "mais apartamento, menos telhado próprio, menos adoção" preveria. A interpretação registrada nos materiais é que essa variável provavelmente funciona como proxy de porte e modernidade urbana (cidades maiores, mais informação, mais instaladoras presentes), não como medida limpa de barreira de telhado individual.
...
Precariedade construtiva: percentual de cortiço, percentual de parede inadequada, percentual/número de população em favela. Verticalização: percentual de domicílios em apartamento (hipótese não confirmada, ver 10.3).
...
- **Jaboatão dos Guararapes, Cabo de Santo Agostinho e Olinda (PE):** irradiação acima da mediana nacional; índice de precariedade habitacional individual (verificado em 26/07/2026): Jaboatão dos Guararapes 0,2241, Cabo de Santo Agostinho 0,2218, Olinda 0,1499; classificados como Vazio de Acesso. Jaboatão dos Guararapes tem, adicionalmente, 644 mil habitantes e recebeu 1 contrato do Reforma Casa Brasil Solar no período auditado (Seção 14).
...
O índice de precariedade habitacional é normalizado min–max em escala nacional (0–1). A escala do índice de segurança da posse usada na fórmula do IVSH é 0–100, confirmada por verificação técnica direta na metodologia implementada (ver inconsistência #3, Bloco 1 — resolvida em 26/07/2026). O Índice de Vulnerabilidade Social consolidado já é, pelos materiais, produzido em escala 0–1.

#### 11.5 Ponderação

**Pesos iguais.** A fórmula documentada nos materiais é uma média aritmética simples dos três componentes — não há ponderação diferenciada entre vulnerabilidade social, precariedade habitacional e insegurança da posse. Um teste de sensibilidade a duas ponderações alternativas (50% vulnerabilidade social / 25% precariedade / 25% posse; e 60% precariedade / 20% social / 20% posse) foi realizado em 26/07/2026: a correlação entre a ordenação nacional original e as duas alternativas é muito alta (0,95 em ambos os casos) — o ranking geral é robusto a variações razoáveis de peso. Olhando só para os 20 municípios de maior IVSH, porém, apenas 9 dos 20 permanecem no top 20 sob qualquer uma das duas alternativas — o topo da lista é mais sensível à escolha de pesos do que o ranking como um todo, embora os cinco municípios já citados publicamente como "top 5" (Seção 11.10) permaneçam todos dentro do top 20 nas duas alternativas testadas.
...
Média nacional: **0,21** (0,2095 na fonte de maior precisão). Intervalo observado: **0 a 0,39** (0 a 0,387 na fonte de maior precisão). Calculado para 5.573 municípios — o universo real e completo da base de municípios do Atlas (ver Seção 6).
...
Índice ainda não validado externamente por pares (Seção 1). Pesos iguais entre dimensões — teste de sensibilidade a duas ponderações alternativas mostrou ranking geral robusto, mas o topo da lista (top 20) menos estável (Seção 11.5). Universo (5.573) e escala do componente de segurança da posse (0–100) — ambos reconciliados em 26/07/2026.
...
|---|---:|---:|---:|---:|
| Índice de Precariedade Habitacional | −0,1312 | **−0,1524** | ≈ 2,7 × 10⁻³⁰ | 5.570 (citado nos materiais; ver nota de universo, Seção 6) |
| Índice de Segurança da Posse | −0,3060 | **−0,2976** | ≈ 3,0 × 10⁻¹¹⁴ | 5.570 |

#### 12.6 Resultados regionais

O sinal do coeficiente parcial é mantido em **4 das 5 regiões brasileiras** para ambas as variáveis. A região divergente foi identificada por reexecução direta do teste com saída desagregada por região (26/07/2026):

| Variável | Região divergente | ρ parcial na região | p-valor | Significância |
|---|---|---:|---:|---|
| Índice de Precariedade Habitacional | Nordeste | +0,0356 | 0,131 | Não significativo — associação praticamente nula, não um sinal forte invertido |
| Índice de Segurança da Posse | Sul | +0,1884 | ≈5,6×10⁻¹¹ | Estatisticamente significativo — inversão real do padrão nacional |

A divergência da Segurança da Posse no Sul confirma, com o teste de três variáveis de controle, a mesma região já apontada por um teste bivariado anterior e metodologicamente distinto (controle só por renda) — não é um achado novo, mas a confirmação de uma suspeita anterior. No Sul do país, ao contrário do padrão nacional, mais segurança da posse está associada a **mais**, não menos, adoção solar residencial — um resultado regional genuinamente distinto do resto do país, cujo mecanismo ainda não foi investigado.
...
Extrato do Sistema de Informação ao Cidadão da Caixa Econômica Federal, cobrindo novembro de 2025 a abril de 2026 (6 meses). Fonte não pública e não automatizável — qualquer atualização futura exige novo pedido formal. Agregado único por município (sem chave de indivíduo, contrato ou endereço). Totais no período: **R$ 61.377.571,09 liberados, 3.253 contratos, 1.093 municípios com pelo menos um contrato**. Confirmado em 26/07/2026, por verificação técnica direta na metodologia implementada: **os valores são nominais**, sem nenhuma etapa de correção monetária aplicada; **o extrato original da Caixa Econômica Federal não traz nenhum campo de status ou cancelamento** — não é possível, com os materiais disponíveis, saber se contratos cancelados já foram excluídos a montante pela Caixa (**[a confirmar diretamente com a fonte]**). A data exata de extração continua **[informação a confirmar]** — só o fim do período coberto (30/04/2026) está registrado.

#### 14.3 Universo

A auditoria opera sobre os mesmos **5.569 municípios classificáveis** da Matriz de Vazios de Acesso (Seção 6): 1.093 com contrato e 4.476 sem contrato (soma exata = 5.569). Uma segunda tabela, na comparação de precariedade habitacional (base completa), usa n=4.480 para "sem contrato" — diferença de 4 municípios, explicada pela exclusão dos 4 municípios sem dado de irradiação, presentes na base de precariedade mas fora do universo classificável de quadrante (ver Seção 6).
...
A soma dos quatro totais (R$ 61.377.571,09) confere exatamente com o total geral liberado no período — quadro internamente consistente. O valor per capita em Vazio de Acesso é **30,6% menor** que em Acesso pleno. Já a comparação com "Adoção acima do potencial" — uma versão preliminar deste texto citava "31% menor", mas o recálculo direto a partir dos mesmos dois valores da tabela (R$ 1.410,93 e R$ 2.441,27) resulta em **42,2% menor**, a divergência foi identificada e corrigida nesta versão.

Quando o programa chega a um Vazio de Acesso, portanto, chega proporcionalmente com **menos recurso por habitante** do que chega a territórios já bem servidos — tanto pela leitura de 30,6% quanto pela de 42,2%, a direção do achado (menos intensidade nos vazios) se sustenta.
...
Testes efetivamente realizados e documentados nos materiais: (i) verificação de robustez regional do sinal dos dois coeficientes parciais da Seção 12 (mantido em 4 de 5 regiões; região divergente identificada em 26/07/2026 como Nordeste para precariedade habitacional — não significativo — e Sul para segurança da posse — significativo, ver Seção 12.6); (ii) teste exploratório de hipóteses alternativas para dois casos regionais divergentes num teste anterior e metodologicamente distinto (Sul/segurança da posse, Centro-Oeste/irradiação), incluindo tipologia habitacional (percentual de domicílios em apartamento) como possível confundidor — não confirmado como explicação, investigação formalmente encerrada sem resultado conclusivo; (iii) separação de MMGD residencial da MMGD total (agronegócio/irrigação), que resolveu 3 dos 4 outliers regionais originalmente observados num teste preliminar.

Testes adicionais realizados em 26/07/2026, por verificação direta na base de dados do Atlas: (iv) sensibilidade da classificação de Vazio de Acesso a um corte por terços em vez de mediana — o conjunto cai de 1.451 para 706 municípios, todos contidos no conjunto original (Seção 9.8); (v) sensibilidade de um eventual corte de verticalização — testados os cortes de 50% (3 municípios no país, nenhum com irradiação alta) e 30% (38 municípios, 8 com irradiação alta) (Seção 10.3); (vi) sensibilidade dos pesos do IVSH a duas ponderações alternativas — ranking geral robusto (correlação de 0,95), mas o top 20 menos estável (9 de 20 municípios permanecem sob as duas alternativas testadas) (Seção 11.5).

Testes **ainda não realizados**: teste formal de dependência ou autocorrelação espacial entre municípios vizinhos; teste de colinearidade entre as variáveis de controle (renda e irradiação); comparação sistemática entre média e mediana para os indicadores financeiros do Reforma Casa Brasil Solar. **[teste de sensibilidade não realizado]** para os itens deste parágrafo.

### 18. Limitações

Limitações das bases: irradiação é uma climatologia de 1999–2015, não uma medição corrente; MMGD residencial é um levantamento acumulado sem série temporal; população municipal é estimada, não censitária; o índice de precariedade habitacional mede material construtivo, não o Déficit Habitacional oficial da Fundação João Pinheiro; o Reforma Casa Brasil Solar é uma fonte estática, obtida por pedido formal de acesso à informação, sem atualização automática.

Diferenças temporais entre fontes: irradiação (1999–2015), moradia (Censo 2022), MMGD (levantamento mais recente disponível), Reforma Casa Brasil Solar (nov/2025–abr/2026) — os três instrumentos combinam dados de períodos distintos sem uma data de referência única.
...
Bases que precisam de atualização periódica, segundo os materiais: MMGD residencial (ANEEL, levantamento acumulado — não há série temporal real hoje; qualquer leitura de "evolução" exigiria novos levantamentos periódicos registrados com data própria); Reforma Casa Brasil Solar (fonte estática, exige novo pedido formal à Caixa Econômica Federal a cada atualização, sem canal público conhecido). Irradiação solar (climatologia INPE/LABREN) e dados de moradia do Censo têm ciclo de atualização próprio, definido pelas respectivas instituições produtoras, não pelo Atlas.
...
Etapas automatizáveis, segundo os materiais: a classificação da Matriz de Vazios de Acesso e o cálculo do IVSH já são processos com metodologia formalmente definida, reexecutáveis sobre dados atualizados. Etapas que exigem revisão humana: qualquer nova extração do Reforma Casa Brasil Solar (fonte manual, documento); reconciliação de eventuais novas inconsistências de universo; diagnóstico da região divergente nos testes de correlação parcial (Seção 12.6); definição de um critério formal de verticalização, caso essa hipótese seja retomada.

Documentação necessária para replicação: os materiais indicam que a metodologia de classificação de quadrante, o cálculo do IVSH e o procedimento de correlação parcial existem como processos formalmente implementados na plataforma do Atlas — mas este anexo não teve acesso à implementação técnica direta, apenas às descrições textuais nos materiais-fonte, e não pode confirmar independentemente que a implementação corresponde exatamente ao texto. **[revisão técnica necessária]** caso se deseje auditar a implementação diretamente antes de publicar este anexo.
...
- IBGE — Cadastro Nacional de Favelas e Comunidades Urbanas.
- RAIS — renda média domiciliar (período não especificado nos materiais).
- Caixa Econômica Federal — extrato do Sistema de Informação ao Cidadão, programa Reforma Casa Brasil Solar, nov/2025–abr/2026.
...
| IVSH | Vulnerabilidade combinada: social + habitacional + posse | Município (n=5.573, universo completo) | IVS consolidado; precariedade habitacional; (1 − segurança da posse/100) | Média simples dos três componentes | Média nacional 0,21; máximo 0,39; top 5 nomeados | Capta vulnerabilidade mesmo fora do quadrante Vazio de Acesso | Sem validação externa por pares; pesos iguais sem teste de sensibilidade |
...
| Intensidade financeira | R$ 1.410,93/1.000 hab. em Vazio de Acesso vs. R$ 2.033,24 (Acesso pleno) e R$ 2.441,27 (Adoção acima do potencial) | Valor liberado per capita por quadrante | Vazios de Acesso recebem recurso proporcionalmente menor | Diferença percentual frente a "Adoção acima do potencial": 42,2%, valor corrigido nesta versão (ver Bloco 1, item 1) |
| Precariedade habitacional | Municípios com contrato têm precariedade ~70% (base completa) a ~51% (dentro dos vazios) maior | Índice de precariedade habitacional, médias por grupo | Correlaciona com precariedade, mas não com o eixo de potencial solar desperdiçado | Sem teste de significância |
...
| Precariedade habitacional associada a menos MMGD, controlando renda e irradiação | ρ parcial = −0,1524, p≈2,7×10⁻³⁰ | Censo 2022 + Cadastro de Favelas e Comunidades Urbanas + ANEEL + RAIS + INPE | Correlação parcial de postos (Spearman) | Nacional, n≈5.570 | Não estabelece causalidade; região divergente é o Nordeste, mas sem significância estatística (Seção 12.6) | Verificado |
| Segurança da posse associada a menos MMGD (sinal invertido) | ρ parcial = −0,2976, p≈3,0×10⁻¹¹⁴ | Censo 2022 + ANEEL + RAIS + INPE | Correlação parcial de postos (Spearman) | Nacional, n≈5.570 | Escala do índice reconciliada (0–100); mecanismo do sinal invertido ainda não identificado | Parcialmente verificado |
| Alta verticalização é um segundo padrão de descompasso morfológico | Nenhum teste direto de confirmação; teste exploratório correlato aponta sinal contrário | Registros técnicos de desenvolvimento da plataforma (achado colateral) | Correlação parcial exploratória, controle por renda | Nacional, mas não desenhado para testar esta hipótese especificamente | Contradiz a narrativa das versões preliminares deste texto | Inconsistência identificada |
| IVSH: média nacional 0,21, máximo 0,39, calculado para 5.573 municípios | Estatísticas descritivas do índice | IVS consolidado + precariedade habitacional + segurança da posse | Média simples dos três componentes | Nacional | 5.573 é o universo real da base de municípios do Atlas (confirmado com a malha territorial oficial do IBGE, sem duplicatas) | Verificado e reconciliado |
| Vazio de Acesso recebe 42,2% menos recurso per capita que "Adoção acima do potencial" | Tabela de valores per capita por quadrante | Extrato do Sistema de Informação ao Cidadão/Caixa | Cálculo de percentual de diferença | Descritivo | Valor citado numa versão preliminar ("31% menor") tinha erro de cálculo — corrigido nesta versão | Verificado e corrigido |
| Programa reforça, não corrige, o padrão espacial de acesso solar | Comparação de frequência e intensidade por quadrante | Extrato do Sistema de Informação ao Cidadão/Caixa + classificação de Vazio de Acesso | Comparação descritiva de percentuais e médias | Descritivo, sem teste formal | Direção do achado (menos presença/intensidade em vazios) se sustenta | Parcialmente verificado |

### QUADRO 5 — Pendências

| Pendência | Por que importa | Informação necessária | Responsável sugerido |
|---|---|---|---|
| ~~Identificar a região que diverge no sinal da correlação parcial~~ **[RESOLVIDO 26/07/2026]** — Nordeste (precariedade, não significativo) e Sul (segurança da posse, significativo) | Uma região com sinal oposto pode indicar mecanismo distinto ou erro de dado localizado | Reexecução do procedimento de correlação com saída desagregada por região — concluída | Concluído |
| ~~Definir (ou descartar formalmente) um critério de corte para "alta verticalização"~~ **[RESOLVIDO 26/07/2026]** — corte de 50% identifica só 3 municípios no país, nenhum com irradiação acima da mediana; corte de 30% identifica 8 | Hoje a hipótese aparece na narrativa sem sustentação estatística nem classificação formal | Teste direcionado da variável de percentual de apartamento como hipótese de barreira, com corte e sensibilidade — concluído | Concluído |
| Corrigir, no rascunho do relatório principal, a divergência de 31% vs. 42,2% na intensidade financeira do Reforma Casa Brasil Solar — **texto de substituição já redigido, ver Bloco 1, item 24** | Número citado publicamente em material de comunicação, com erro de cálculo já identificado e corrigido nesta nota | Localizar o trecho no rascunho do relatório principal e colar o texto corrigido já preparado | [responsável a definir] |
| Corrigir, no rascunho do relatório principal, o termo "ortogonal" — **texto de substituição já redigido, ver Bloco 1, item 25** | Reivindica um teste estatístico de independência que não foi feito | Localizar o trecho e colar o texto corrigido já preparado | [responsável a definir] |
| Inserir, no rascunho do relatório principal, o achado do sinal invertido da segurança da posse — **parágrafo pronto, ver Bloco 1, item 26** | Omitir um resultado inesperado que os próprios materiais internos preservam reduz a transparência que o relatório reivindica | Inserir o parágrafo já redigido logo após o achado de precariedade habitacional | [responsável a definir] |
| Completar objetivos, metas territoriais e condições de crédito do Reforma Casa Brasil Solar (renda até R$ 9.600/mês e dispensa de escritura já confirmados no relatório principal; faltam metas territoriais, focalização ativa, juros/prazo, maturação) | Sem isso, a auditoria de aderência territorial não pode virar avaliação de eficácia | Documento oficial do programa — pedido formal à Caixa Econômica Federal ou ao Ministério das Cidades | [responsável a definir — Instituto Pólis, contato institucional] |
| Confirmar a existência de contratos cancelados/duplicados no extrato do programa, e a data exata de extração | O extrato disponível não tem campo de status — não é possível verificar isso com os materiais atuais | Confirmação junto à Caixa Econômica Federal (Sistema de Informação ao Cidadão) sobre o extrato já recebido | [responsável a definir — Instituto Pólis, contato institucional] |
| ~~Confirmar se valores financeiros do programa são nominais ou corrigidos~~ **[RESOLVIDO 26/07/2026]** — nominais, sem etapa de correção monetária | Afeta comparabilidade de qualquer análise temporal futura | Verificação direta na metodologia implementada — concluída | Concluído |
| Renomear a seção "Resultados Ineficientes de Crédito" do relatório principal (Parte III, item 4) — **novo título já sugerido, ver Bloco 1, item 22** | O título afirma "ineficiência" antes de os critérios oficiais do programa terem sido plenamente confrontados com a distribuição observada | Decisão editorial da equipe de redação | [responsável a definir] |
...
- Dados necessários: classificação municipal de quadrante (já existente, segundo os materiais, na base de dados do Atlas).
...
- Mensagem principal: menor intensidade financeira per capita em Vazio de Acesso — usar o valor recalculado (42,2%) e, se o valor antigo aparecer em algum material citado, sinalizar a correção em nota de rodapé do próprio gráfico, não silenciosamente.
...
- Dados necessários: nenhum dado adicional — é uma representação do desenho metodológico já descrito neste anexo.
...
**Nota sobre o texto acima:** ele cita o valor per capita do Reforma Casa Brasil Solar com o percentual já corrigido (42,2%, ver Seção 14.5 e Bloco 1, item 1) — o valor "31% menor", presente numa versão preliminar deste texto, continha um erro de cálculo já sanado nesta versão.
...
**Trechos fundidos:** as descrições da Matriz de Vazios de Acesso, do Descompasso Morfológico e do IVSH, que apareciam de forma parcialmente duplicada em versões preliminares deste texto (uma voltada ao detalhamento técnico, outra voltada a tomadores de decisão), foram unificadas numa única descrição por instrumento neste anexo, preservando os números de ambas quando coincidentes e sinalizando divergências quando não coincidiam (ex.: mediana de irradiação 5,02 vs. 5,015 kWh/m²/dia — tratada como diferença de arredondamento, não de fonte).

**Repetições eliminadas:** a narrativa de "moradia como pré-condição, não substituta da renda" aparecia de forma quase idêntica nas versões preliminares — mantida uma única vez, na Seção 4 do anexo (Referencial conceitual).
...
**Informações mantidas apenas neste anexo, não no corpo principal do relatório:** a tabela completa de correlação parcial com controles (Seção 12.5), os quadros de rastreabilidade (Quadro 4) e de pendências (Quadro 5), a descrição completa da metodologia de normalização e ponderação do IVSH, e o detalhamento das inconsistências numéricas identificadas no Bloco 1.

**Lacunas que exigem informação externa, sem solução possível apenas com a base de dados do Atlas:** objetivos, metas territoriais e condições de crédito oficiais do Reforma Casa Brasil Solar (dependem de resposta da Caixa Econômica Federal ou do Ministério das Cidades); confirmação de contratos cancelados/duplicados e data exata de extração do extrato do programa (dependem de resposta da Caixa Econômica Federal); mecanismo causal por trás da inversão de sinal da segurança da posse na região Sul. (Escala do índice de segurança da posse, universo do IVSH, corte de verticalização e região divergente do teste de correlação parcial — todos reconciliados em 26/07/2026 por verificação direta na base de dados do Atlas, ver Bloco 1.)

**Verificações necessárias antes da publicação, em ordem de prioridade sugerida:**
1. Obter, junto à Caixa Econômica Federal ou ao Ministério das Cidades, os objetivos oficiais e critérios de elegibilidade do Reforma Casa Brasil Solar.
2. Corrigir, no rascunho do relatório principal, a mesma divergência de 31% vs. 42,2% já sanada nos documentos internos (ver Bloco 1, item 24).
3. Identificar nominalmente a região que diverge no sinal da correlação parcial de três variáveis de controle (Seção 12.6).
4. Definir, ou descartar formalmente, um critério de corte para a hipótese de "alta verticalização" (Seção 10.3).

**Análises já realizadas em 26/07/2026** (ver itens correspondentes do Bloco 1 e as seções técnicas indicadas): teste de corte/sensibilidade para a hipótese de verticalização (Seção 10.3); diagnóstico nominal da região divergente na correlação parcial de três variáveis, para as duas variáveis de moradia (Seção 12.6); teste de sensibilidade dos pesos do IVSH e do corte por mediana da Matriz de Vazios de Acesso (Seções 9.8 e 11.5).

**Análises que continuam recomendadas, não realizadas nesta verificação:** cruzamento numérico direto entre IVSH e cobertura do Reforma Casa Brasil Solar (hoje só indireto, por meio de casos individuais); investigação do mecanismo causal por trás da inversão de sinal da segurança da posse na região Sul; teste formal de autocorrelação espacial entre municípios vizinhos; teste de colinearidade entre renda e irradiação.
